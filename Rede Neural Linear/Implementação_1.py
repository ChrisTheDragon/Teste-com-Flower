...
train_dataloader = DataLoader(train_dataset, batch_size=nAmostras_treino, shuffle=True)
test_dataloader = DataLoader(test_dataset, batch_size=nAmostras_treino, shuffle=True)

# Verificando se há disponibilidade de GPU
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

# Criando a rede neural
modelo = RN.LineNetwork().to(device)
...
optimize = torch.optim.SGD(modelo.parameters(), lr=1e-2)
